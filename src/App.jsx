...
// ─── QUICK PRODUCT MODAL (salva direto no Supabase) ───────────
...
export default function App() {
  ...
  // ══════════════════════════════════════════════════════════════
  // MOBILE: forms e detalhes (early return)
  // ══════════════════════════════════════════════════════════════
  ...
  if (view === "rForm") return (<div style={s.app}><header style={s.hdr}><button style={s.bback} onClick={() => setView(editId?"detail":"list")}>← Cancelar</button><span style={s.logo}>{editId?"Editar Receita":"Nova Receita"}</span><div style={{width:70}}/></header><ReceitaForm initialData={rFormInit} editId={editId} produtos={produtos} saving={saving} onSaved={handleSaveRecipe} onOpenQuickP={() => setQuickPOpen(true)} toast_={toast_}/><QuickProdModal open={quickPOpen} onClose={() => setQuickPOpen(false)} negocioId={negocioId} onProductSaved={reloadProdutos} toast_={toast_}/><ModalConfirm item={confirmDel} onConfirm={confirmarExc} onCancel={() => setConfirmDel(null)}/>{toast && <div style={s.tst}>{toast}</div>}</div>);

  if (view === "cDetail" && compraDetalhe) {
    const c = compras.find(x => x.id === compraDetalhe); if (!c) { setView("list"); return null; }
    const it = compraItens[compraDetalhe] || []; const tot = it.reduce((sum, i) => sum + parseFloat(i.valor_subtotal||0), 0);
    return (<div style={s.app}><div style={s.dh}><button style={s.bback} onClick={() => {setView("list");setTab("compras");}}>← Voltar</button><div style={{fontSize:44,marginBottom:7,marginTop:10}}>🛒</div><div style={s.dn}>{c.fornecedor || "Compra"}</div><div style={s.dc}>{new Date(c.data_compra+"T12:00:00").toLocaleDateString("pt-BR")} · {c.forma_pagamento||""} · {mapaUsuarios[c.created_by]||""}</div></div><div style={s.db}><div style={s.sec}><div style={s.st}>📦 Itens ({it.length})</div>{it.map((i,idx) => <div key={idx} style={s.ir}><div><div style={{fontWeight:500,fontSize:13}}>{i.produto_nome_snapshot}</div><div style={{fontSize:11,color:G,marginTop:1}}>{i.qtd} {i.unidade} × {fmt(i.valor_unitario)}</div></div><div style={s.icost}>{fmt(i.valor_subtotal)}</div></div>)}<div style={{...s.ir,borderTop:`2px solid ${RC}`,marginTop:5,paddingTop:7,fontWeight:700}}><div>Total</div><div style={s.icost}>{fmt(tot)}</div></div></div>{c.obs && <div style={s.sec}><div style={s.st}>📝 Obs</div><div style={{fontSize:13,color:G,lineHeight:1.6}}>{c.obs}</div></div>}<button style={s.bd} onClick={() => pedirExcC(c.id)}>🗑 Excluir compra</button></div><ModalConfirm item={confirmDel} onConfirm={confirmarExc} onCancel={() => setConfirmDel(null)}/>{toast && <div style={s.tst}>{toast}</div>}</div>);
  }
  ...
  // Lista compartilhada (usada mobile e sidebar desktop)
  ...
  // Sidebar list for desktop (dark theme)
  const sideList = (
