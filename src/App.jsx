...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .card-hover:hover{background:${V}22!important;cursor:pointer;}
  .card-hover-light:hover{box-shadow:0 4px 16px rgba(74,26,44,.13)!important;transform:translateY(-1px);cursor:pointer;}
  .btn-hover:hover{opacity:.85;cursor:pointer;}
  ...
  @media(min-width:768px){
    ...
    .desktop-grid{display:grid!important;grid-template-columns:260px 1fr;height:100vh;overflow:hidden;}
    .desktop-sidebar{background:${V};color:#fff;display:flex;flex-direction:column;overflow:hidden;}
    .desktop-content{background:${CR};overflow-y:auto;display:flex;flex-direction:column;}
    .desktop-detail-grid{display:grid;grid-template-columns:1fr 340px;gap:24px;align-items:start;}
  }
`;

// ─── ESTILOS MOBILE (idênticos à versão anterior) ──────────────────────────
const s = {
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
      <div style={{background:W,borderRadius:18,padding:22,width:"100%",maxWidth:360,textAlign:"center"}}>
        ...
        <div style={{fontSize:13,color:G,marginBottom:20,lineHeight:1.6}}>
          Excluir <b>"{item.nome}"</b>?<br/><span style={{fontSize:11,color:R}}>Esta ação não pode ser desfeita.</span>
        </div>
...
function DesktopDetail({ r, produtos, negocioNome, onEdit, onCopy, onDelete }) {
  ...
  return (
    <div style={{padding:"28px 32px",animation:"fadein .2s ease"}} className="print-area">
      {/* TOPO */}
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:20}} className="no-print">
        ...
        <div style={{display:"flex",gap:8,flexShrink:0}}>
          <button className="btn-hover" style={{padding:"8px 18px",borderRadius:8,border:`2px solid ${RC}`,background:W,color:V,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}} onClick={onEdit}>✏️ Editar</button>
          <button className="btn-hover" style={{padding:"8px 18px",borderRadius:8,border:"none",background:V,color:W,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",display:"flex",alignItems:"center",gap:6}} onClick={handlePrint}>🖨️ Imprimir Ficha</button>
        </div>
      </div>

      {/* PRINT HEADER — só aparece na impressão */}
      <div className="print-only" style={{display:"none",marginBottom:24,borderBottom:`2px solid ${RC}`,paddingBottom:16}}>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:22,color:V,fontWeight:700}}>{negocioNome}</div>
        <div style={{fontSize:13,color:G,marginTop:4}}>Ficha Técnica — {r.nome}</div>
      </div>
      <div className="print-only" style={{display:"none",marginBottom:20}}>
        <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:26,color:V}}>{r.emoji} {r.nome}</h1>
        <div style={{display:"flex",gap:12,marginTop:6}}>
          <span style={{background:RC,color:V,borderRadius:20,padding:"3px 12px",fontSize:12,fontWeight:600}}>Rendimento: {r.rendimento} unidades</span>
          {r.categoria && <span style={{background:`${R}18`,color:R,borderRadius:20,padding:"3px 12px",fontSize:12,fontWeight:600}}>Categoria: {r.categoria}</span>}
        </div>
      </div>

      {/* GRID PRINCIPAL */}
      <div className="desktop-detail-grid" style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:24,alignItems:"start"}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead>
              <tr style={{borderBottom:`2px solid ${RC}`}}>
                <th style={{textAlign:"left",padding:"6px 8px 10px 0",fontSize:11,textTransform:"uppercase",letterSpacing:".05em",color:G,fontWeight:600}}>Ingrediente</th>
                <th style={{textAlign:"right",padding:"6px 8px 10px",fontSize:11,textTransform:"uppercase",letterSpacing:".05em",color:G,fontWeight:600}}>Qtd Usada</th>
                <th style={{textAlign:"right",padding:"6px 8px 10px",fontSize:11,textTransform:"uppercase",letterSpacing:".05em",color:G,fontWeight:600}}>Custo Base</th>
                <th style={{textAlign:"right",padding:"6px 0 10px",fontSize:11,textTransform:"uppercase",letterSpacing:".05em",color:G,fontWeight:600}}>Total na Receita</th>
              </tr>
            </thead>
            <tbody>
              {r.ingredientes.map((ing,i) => {
                ...
                if (!p) return null;
                const custo = (p.preco / p.embalagemQtd) * ing.usadoQtd;
                return (
                  <tr key={i} style={{borderBottom:`1px solid ${RC}`}}>
                    <td style={{padding:"10px 8px 10px 0",color:V,fontWeight:500}}>{p.nome}</td>
                    <td style={{padding:"10px 8px",textAlign:"right",color:G}}>{ing.usadoQtd}{p.unidade}</td>
                    <td style={{padding:"10px 8px",textAlign:"right",color:G}}>{fmt(p.preco)} / {p.embalagemQtd}{p.unidade}</td>
                    <td style={{padding:"10px 0",textAlign:"right",color:V,fontWeight:600}}>{fmt(custo)}</td>
                  </tr>
                );
              })}
            </tbody>
            <tfoot>
              <tr style={{borderTop:`2px solid ${RC}`}}>
                <td colSpan={3} style={{padding:"12px 8px 6px 0",fontWeight:700,color:V}}>Custo dos Ingredientes</td>
                <td style={{padding:"12px 0 6px",textAlign:"right",fontWeight:700,color:R}}>{fmt(ci)}</td>
              </tr>
              <tr>
                <td colSpan={3} style={{padding:"4px 8px 6px 0",color:G,fontSize:12}}>Outros custos ({r.outrosCustos||30}% sobre ingredientes)</td>
                <td style={{padding:"4px 0 6px",textAlign:"right",color:G,fontSize:12}}>{fmt(outros)}</td>
              </tr>
              {r.despesas > 0 && <tr>
                <td colSpan={3} style={{padding:"4px 8px 6px 0",color:G,fontSize:12}}>Despesas extras</td>
                <td style={{padding:"4px 0 6px",textAlign:"right",color:G,fontSize:12}}>{fmt(r.despesas)}</td>
              </tr>}
              <tr style={{borderTop:`2px solid ${RC}`}}>
                <td colSpan={3} style={{padding:"10px 8px 0 0",fontWeight:700,fontSize:14,color:V}}>Custo Total (com perdas e extras)</td>
                <td style={{padding:"10px 0 0",textAlign:"right",fontWeight:700,fontSize:14,color:R}}>{fmt(total)}</td>
              </tr>
            </tfoot>
          </table>
          {r.obs && <div style={{marginTop:16,paddingTop:14,borderTop:`1px solid ${RC}`}}>
            <div style={{fontSize:11,fontWeight:600,color:G,textTransform:"uppercase",letterSpacing:".05em",marginBottom:6}}>Observações</div>
            <div style={{fontSize:13,color:G,lineHeight:1.6}}>{r.obs}</div>
          </div>}
        </div>
        ...
        <div style={{background:W,borderRadius:14,padding:"20px",boxShadow:`0 2px 12px rgba(74,26,44,.08)`}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <div style={s.pb}><div style={s.pl}>Custo / Unidade</div><div style={{...s.pv,fontSize:16}}>{fmt(porUn)}</div></div>
            <div style={s.pb}><div style={s.pl}>Margem Desejada</div><div style={{...s.pv,fontSize:16}}>{r.margem}%</div></div>
            <div style={s.pb}><div style={s.pl}>Preço s/ Taxa</div><div style={{...s.pv,fontSize:16}}>{fmt(semT)}</div></div>
            <div style={s.pb}><div style={s.pl}>Taxa Delivery ({r.taxaDelivery}%)</div><div style={{...s.pv,fontSize:16}}>{fmt(taxa)}</div></div>
          </div>
          <div style={{background:`linear-gradient(135deg,${R},${RL})`,borderRadius:12,padding:"16px",color:W,marginBottom:8,textAlign:"center"}}>
            ...
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:30,fontWeight:700}}>{fmt(final)}</div>
          ...
          <div style={{background:"#FFFBEB",border:"1.5px solid #FCD34D",borderRadius:10,padding:"12px 14px",marginBottom:8}}>
            ...
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:"#92400E"}}>{fmt(lucro)}</div>
          ...
          {r.precoApp > 0 && <>
            <div style={{background:"#F0FFF4",border:"1.5px solid #86EFAC",borderRadius:10,padding:"12px 14px",marginBottom:8}}>
              ...
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:"#166534"}}>{fmt(r.precoApp)}</div>
            </div>
            {lucroApp !== null && <div style={{background:"#F0FFF4",border:"1.5px solid #86EFAC",borderRadius:10,padding:"12px 14px"}}>
              ...
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:"#166534"}}>{fmt(lucroApp)}</div>
            </div>}
          </>}
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:8,marginTop:16}} className="no-print">
        <button className="btn-hover" style={{padding:"9px 16px",borderRadius:8,border:`2px solid ${RC}`,background:W,color:G,fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}} onClick={onCopy}>📋 Copiar receita</button>
        <button className="btn-hover" style={{padding:"9px 16px",borderRadius:8,border:`2px solid ${RC}`,background:W,color:R,fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}} onClick={onDelete}>🗑 Excluir</button>
      </div>
    </div>
  );
}

// ─── QUICK PRODUCT MODAL (componente externo — evita perda de foco) ───────────
function QuickProdModal({ quickP, setQuickP, produtos, saveP, toast_ }) {
  const [form, setForm] = useState({ id:"", nome:"", preco:"", embalagemQtd:"", unidade:"g", categoria:"Secos" });

  useEffect(() => {
    if (quickP) setForm({ ...quickP });
  }, [quickP?.id]); // só reinicia quando abre um novo modal

  if (!quickP) return null;
  const pu = form.preco && form.embalagemQtd ? parseFloat(form.preco) / parseFloat(form.embalagemQtd) : null;
  const inp = { width:"100%", padding:"10px 12px", borderRadius:9, border:`2px solid ${RC}`, background:CR, fontFamily:"'DM Sans',sans-serif", fontSize:14, color:V, outline:"none", marginBottom:10, boxSizing:"border-box", WebkitAppearance:"none" };
  const lbl = { display:"block", fontSize:10, fontWeight:600, color:G, textTransform:"uppercase", letterSpacing:".05em", marginBottom:4 };
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:300,display:"flex",alignItems:"flex-end"}} onClick={() => setQuickP(null)}>
      <div style={{background:W,borderRadius:"18px 18px 0 0",padding:18,width:"100%",maxWidth:480,margin:"0 auto",maxHeight:"90vh",overflowY:"auto"}} onClick={e => e.stopPropagation()}>
        ...
        <input style={inp} placeholder="Ex: Chocolate 70%" value={form.nome} onChange={e => setForm(f=>({...f,nome:e.target.value}))} autoFocus/>
        ...
        <div style={{display:"flex",gap:10}}>
          <div style={{flex:1}}>
            ...
            <input style={inp} type="number" step="0.01" placeholder="10.00" value={form.preco} onChange={e => setForm(f=>({...f,preco:e.target.value}))}/>
          </div>
          <div style={{flex:1}}>
        ...
        <label style={lbl}>Qtd na embalagem ({form.unidade}) *</label>
        <input style={inp} type="number" step="any" placeholder="200" value={form.embalagemQtd} onChange={e => setForm(f=>({...f,embalagemQtd:e.target.value}))}/>
        {pu !== null && <div style={{background:`${R}18`,borderRadius:7,padding:"5px 9px",fontSize:11,color:R,fontWeight:600,marginBottom:7}}>💡 Custo: R$ {pu.toFixed(4).replace(".",",")} /{form.unidade}</div>}
        <div style={{display:"flex",gap:10,marginTop:4}}>
          <button style={{background:`linear-gradient(135deg,${R},${RL})`,color:W,border:"none",borderRadius:50,padding:15,flex:1,fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:600,cursor:"pointer"}} onClick={async () => {
            if (!form.nome.trim()||!form.preco||!form.embalagemQtd) { toast_("⚠️ Preencha todos os campos"); return; }
            const c = {...form, id: Date.now().toString(), preco:parseFloat(form.preco)||0, embalagemQtd:parseFloat(form.embalagemQtd)||1};
            const up = [...produtos, c]; await saveP(up);
            setQuickP(null); toast_("✅ Produto cadastrado!");
          }}>Salvar Produto</button>
          <button style={{background:"none",border:`2px solid ${RC}`,color:R,borderRadius:50,padding:"13px 16px",fontSize:13,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}} onClick={() => setQuickP(null)}>Cancelar</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [view,       setView]       = useState("list");   // list | rForm | pForm | detail
  ...
  async function setupNegocio(userId, userMeta) {
    const { data: membro, error: errMembro } = await supabase
      .from("membros").select("negocio_id").eq("user_id", userId).maybeSingle();
    if (errMembro) throw new Error("Erro ao buscar membro: " + errMembro.message);
    if (membro?.negocio_id) {
      const { data: negocio } = await supabase
        .from("negocios").select("nome").eq("id", membro.negocio_id).maybeSingle();
    ...
    const { data: negocio, error: errNeg } = await supabase
      .from("negocios").insert({ nome: nomeNegocio }).select("id").single();
    if (errNeg) throw new Error("Erro ao criar negócio: " + errNeg.message);
    const { error: errMem } = await supabase
      .from("membros").insert({ negocio_id: negocio.id, user_id: userId, papel: "dono" });
  ...
  async function loadUserData(sessionUser) {
    ...
    const timeout = setTimeout(async () => {
      ...
      try { Object.keys(localStorage).forEach(k => { if (k.startsWith("sb-") || k.includes("delicias-jay-auth") || k.includes("supabase")) localStorage.removeItem(k); }); } catch (e) {}
    ...
    try {
      ...
      setRecipes(SEED_RECIPES); setProdutos(SEED_PRODUTOS);
    }
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "SIGNED_OUT") {
        ...
        setRecipes([]); setProdutos([]);
        setReceitasRowId(null); setProdutosRowId(null);
        setView("list"); setTab("receitas"); setAppReady(true);
        return;
      }
      if (event === "TOKEN_REFRESHED" || event === "USER_UPDATED") return;
      if ((event === "SIGNED_IN" || event === "INITIAL_SESSION") && session?.user) {
        setUser(session.user);
        if (!dadosCarregados) { dadosCarregados = true; await loadUserData(session.user); }
  ...
  const confirmarExc = async () => {
    ...
    if (confirmDel.tipo === "receita") {
      ...
      toast_("🗑 Receita excluída"); setView("list"); setTab("receitas"); setDetailId(null);
    } else {
  ...
  const exportarCSV = (tipo) => {
    ...
    if (tipo === "receitas") {
      nome = "receitas.csv"; csv = "Nome;Categoria;Rendimento;Margem%;Taxa Delivery%;Custo Total;Custo/Unidade;Preço Sugerido/Un;Preço Praticado;Lucro Estimado Lote\n";
      ...
      nome = "produtos.csv"; csv = "Nome;Categoria;Unidade;Preço Embalagem;Qtd Embalagem;Custo por Unidade;Usado em Receitas\n";
  ...
  // TELAS DE AUTH
  // ══════════════════════════════════════════════════════════════
  const authInputStyle = (hasError) => ({ width:"100%", padding:"13px 18px", borderRadius:50, border:`2px solid ${hasError?"#ff6b6b":"#C45C7444"}`, background:"rgba(255,255,255,.15)", fontSize:15, color:W, WebkitTextFillColor:W, outline:"none", textAlign:"center", boxSizing:"border-box", marginBottom:10, fontFamily:"'DM Sans',sans-serif" });
  ...
  if (!user && authView === "emailConfirm") return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:V}}>
      ...
      <div style={{textAlign:"center",padding:"0 32px",width:"100%",maxWidth:380}}>
        ...
        <div style={{color:`${RL}bb`,fontSize:14,lineHeight:1.7,marginBottom:28}}>Enviamos um link de confirmação para<br/><strong style={{color:RL}}>{authForm.email}</strong><br/><br/>Clique no link para ativar sua conta e volte aqui para entrar.</div>
  ...
  if (!user && authView === "cadastro") return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:V}}>
      ...
      <div style={{textAlign:"center",padding:"0 32px",width:"100%",maxWidth:380}}>
        ...
        <input style={authInputStyle(!!authError)} type="text" placeholder="Nome do seu negócio 🏪" value={authForm.nomeNegocio} onChange={e => { setAuthForm({...authForm, nomeNegocio: e.target.value}); setAuthError(""); }}/>
        <input style={authInputStyle(!!authError)} type="email" placeholder="Seu email" value={authForm.email} onChange={e => { setAuthForm({...authForm, email: e.target.value}); setAuthError(""); }}/>
        <input style={authInputStyle(!!authError)} type="password" placeholder="Senha (mín. 6 caracteres)" value={authForm.senha} onChange={e => { setAuthForm({...authForm, senha: e.target.value}); setAuthError(""); }} onKeyDown={e => e.key === "Enter" && fazerCadastro()}/>
  ...
  if (!user) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:V}}>
      ...
      <div style={{textAlign:"center",padding:"0 32px",width:"100%",maxWidth:380}}>
        ...
        <input style={authInputStyle(!!authError)} type="email" placeholder="Seu email" value={authForm.email} onChange={e => { setAuthForm({...authForm, email: e.target.value}); setAuthError(""); }}/>
        <input style={{...authInputStyle(!!authError), letterSpacing:".15em", animation:authError?"shake .3s":""}} type="password" placeholder="Senha" value={authForm.senha} onChange={e => { setAuthForm({...authForm, senha: e.target.value}); setAuthError(""); }} onKeyDown={e => e.key === "Enter" && fazerLogin()}/>
        {authError && <div style={{color:"#ff8fa3",fontSize:12,marginBottom:10}}>{authError}</div>}
  ...
  // ══════════════════════════════════════════════════════════════
  // FORM PRODUTO (mobile + desktop via modal/drawer)
  // ══════════════════════════════════════════════════════════════
  const ProdutoForm = () => {
    ...
    return (
      <div style={s.fb}>
        ...
        <div style={s.sec}>
                ...
                <div style={{position:"relative",marginBottom:p?0:10}}>
                  {p ? (
                    <div style={{display:"flex",alignItems:"center",gap:7,background:"#F0FFF4",border:"2px solid #86EFAC",borderRadius:9,padding:"9px 12px"}}>
                      ...
                      <button style={{background:"none",border:"none",color:R,fontSize:15,cursor:"pointer",padding:0,flexShrink:0}} onClick={() => { const a=[...rForm.ingredientes]; a[idx]={...a[idx],produtoId:"",_busca:""}; setRForm({...rForm,ingredientes:a}); }}>✕</button>
                    </div>
                  ) : (
                    <>
                      <input style={{...s.inp,marginBottom:0,paddingLeft:34,background:CR,borderColor:RC}} placeholder="Digite para buscar... ex: margarina" value={bsc} onChange={e => { const a=[...rForm.ingredientes]; a[idx]={...a[idx],_busca:e.target.value,produtoId:""}; setRForm({...rForm,ingredientes:a}); }}/>
                      ...
                      {res.length > 0 && (
                        <div style={{position:"absolute",top:"100%",left:0,right:0,background:W,border:`2px solid ${RC}`,borderRadius:10,zIndex:50,boxShadow:"0 8px 20px rgba(74,26,44,.15)",maxHeight:190,overflowY:"auto"}}>
                          {res.map(pr => (
                            <div key={pr.id} style={{padding:"9px 12px",cursor:"pointer",borderBottom:`1px solid #FDE8ED`,fontSize:12}} onClick={() => { const a=[...rForm.ingredientes]; a[idx]={...a[idx],produtoId:pr.id,_busca:pr.nome}; setRForm({...rForm,ingredientes:a}); }}>
          ...
          <button style={{...s.badd,marginTop:7,borderColor:"#86EFAC55",color:"#166534",background:"#F0FFF4"}} onClick={() => setQuickP({id:Date.now().toString(),nome:"",preco:"",embalagemQtd:"",unidade:"g",categoria:"Secos"})}>📦 Cadastrar novo produto</button>
        </div>
        <div style={s.sec}>
          ...
          <div style={{display:"flex",gap:10}}>
            <div style={{flex:1}}>
              ...
              <input style={{...s.inp,fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:R,textAlign:"center"}} type="number" min={0} step={1} placeholder="100" value={rForm.margem||""} onChange={e => setRForm({...rForm,margem:parseFloat(e.target.value)||0})}/>
            </div>
            <div style={{flex:1}}>
              <label style={s.lbl}>Taxa delivery (%)</label>
              <input style={{...s.inp,fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:R,textAlign:"center"}} type="number" min={0} step={1} placeholder="30" value={rForm.taxaDelivery||""} onChange={e => setRForm({...rForm,taxaDelivery:parseFloat(e.target.value)||0})}/>
            </div>
          </div>
          <label style={s.lbl}>Outros custos — gás, energia, etc. (%)</label>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            <input style={{...s.inp,marginBottom:0,width:90,fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700,color:R,textAlign:"center"}} type="number" min={0} step={1} placeholder="30" value={rForm.outrosCustos!==undefined?rForm.outrosCustos:30} onChange={e => setRForm({...rForm,outrosCustos:parseFloat(e.target.value)||0})}/>
    ...
    <div style={s.app} className="mobile-only">
      <header style={s.hdr}>
        ...
        <div style={{display:"flex",gap:7,alignItems:"center"}}>
          ...
          {view !== "list" && <button style={s.bback} onClick={() => { if(view==="detail") { setView("list"); } else if(view==="rForm") { setView(editId?"detail":"list"); } else { setView("list"); } }}>← Voltar</button>}
      ...
      {view === "list" && <>
        ...
        {tab === "receitas" && <>
          ...
          {filtR.length === 0
            ...
            : <div style={s.list}>{filtR.map(r => { const { final } = calc(r, produtos); return (
                <div key={r.id} style={s.card} className="card-hover-light" onClick={() => openDet(r.id)}>
        ...
        {tab === "produtos" && <>
          ...
          <div style={s.list}>{filtP.map(p => {
            ...
            return (
              <div key={p.id} style={s.card} className="card-hover-light" onClick={() => openEditP(p.id)}>
  ...
  // ══════════════════════════════════════════════════════════════
  // DESKTOP — layout novo inspirado na referência
  // ══════════════════════════════════════════════════════════════
  const DesktopApp = () => (
    <div className="desktop-only desktop-grid" style={{height:"100vh",overflow:"hidden"}}>

      {/* SIDEBAR ESCURA */}
      <div className="desktop-sidebar" style={{background:V,display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden"}}>
        {/* Logo */}
        ...
        <div style={{padding:"10px 12px 6px",position:"relative"}}>
          ...
          <input
            style={{width:"100%",padding:"8px 10px 8px 30px",borderRadius:8,border:"1px solid rgba(255,255,255,.15)",background:"rgba(255,255,255,.08)",fontSize:12,color:W,WebkitTextFillColor:W,outline:"none",boxSizing:"border-box",fontFamily:"'DM Sans',sans-serif"}}
        ...
        <div style={{display:"flex",gap:5,padding:"4px 12px 8px",overflowX:"auto",scrollbarWidth:"none"}}>
          {(tab==="receitas"?["Todos",...CAT_R]:["Todos",...CAT_P]).map(c => (
            <button key={c} style={{flexShrink:0,padding:"3px 10px",borderRadius:20,border:"1px solid rgba(255,255,255,.2)",background:(tab==="receitas"?catR:catP)===c?"rgba(255,255,255,.2)":"transparent",color:(tab==="receitas"?catR:catP)===c?W:"rgba(255,255,255,.5)",fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}
              onClick={() => tab==="receitas"?setCatR(c):setCatP(c)}>
              {c}
            </button>
          ))}
        </div>
          ...
          {tab === "receitas" && (
            filtR.length === 0
              ...
              : filtR.map(r => { const { final } = calc(r, produtos); return (
                <div key={r.id} className="card-hover" style={{padding:"10px 10px",borderRadius:10,marginBottom:4,display:"flex",alignItems:"center",gap:10,background:detailId===r.id&&view==="detail"?"rgba(255,255,255,.15)":"transparent",transition:"all .15s"}} onClick={() => openDet(r.id)}>
                  <span style={{fontSize:20,flexShrink:0}}>{r.emoji}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{color:W,fontWeight:600,fontSize:13,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{r.nome}</div>
                    <div style={{color:"rgba(255,255,255,.4)",fontSize:10,marginTop:1}}>{r.categoria||"Sem categoria"}</div>
                  </div>
          ...
          {tab === "produtos" && (
            filtP.length === 0
              ...
              : filtP.map(p => { const pu = p.embalagemQtd ? p.preco / p.embalagemQtd : 0; const used = recipes.filter(r => r.ingredientes.some(i => i.produtoId === p.id)).length; return (
                <div key={p.id} className="card-hover" style={{padding:"10px",borderRadius:10,marginBottom:4,display:"flex",alignItems:"center",gap:10,background:editPId===p.id&&view==="pForm"?"rgba(255,255,255,.15)":"transparent",transition:"all .15s"}} onClick={() => openEditP(p.id)}>
                  <span style={{fontSize:18,flexShrink:0}}>{CATEMOJI[p.categoria]||"📦"}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{color:W,fontWeight:600,fontSize:13,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{p.nome}</div>
                    <div style={{color:"rgba(255,255,255,.4)",fontSize:10,marginTop:1}}>{p.categoria} · R$ {fmtN(pu)}/{p.unidade}{used>0?` · ${used} receita${used>1?"s":""}`:" "}</div>
                  </div>
        ...
        <div style={{padding:"12px",borderTop:"1px solid rgba(255,255,255,.1)",display:"flex",gap:6}}>
          <button className="btn-hover" style={{flex:1,padding:"8px",borderRadius:8,border:"none",background:R,color:W,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}} onClick={tab==="receitas"?openNewR:openNewP}>+ {tab==="receitas"?"Receita":"Produto"}</button>
          <button className="btn-hover" style={{padding:"8px 10px",borderRadius:8,border:"1px solid rgba(255,255,255,.2)",background:"transparent",color:"rgba(255,255,255,.6)",fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}} onClick={() => setViewRel(true)} title="Relatórios">📊</button>
          <button className="btn-hover" style={{padding:"8px 10px",borderRadius:8,border:"1px solid rgba(255,255,255,.2)",background:"transparent",color:"rgba(255,255,255,.6)",fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}} onClick={fazerLogout} title="Sair">🚪</button>
        </div>
      </div>

      {/* PAINEL DIREITO */}
      <div style={{background:CR,overflowY:"auto",display:"flex",flexDirection:"column"}}>
        {view === "list" && tab === "receitas" && (
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%",flexDirection:"column",gap:12,color:`${G}88`}}>
            <div style={{fontSize:64}}>🍰</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:22,color:G}}>Selecione uma receita</div>
            <div style={{fontSize:13,color:`${G}66`}}>ou clique em + Receita para criar</div>
          </div>
        )}
        {view === "list" && tab === "produtos" && (
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%",flexDirection:"column",gap:12,color:`${G}88`}}>
            <div style={{fontSize:64}}>📦</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:22,color:G}}>Selecione um produto</div>
            <div style={{fontSize:13,color:`${G}66`}}>ou clique em + Produto para criar</div>
          </div>
        )}
        {view === "detail" && (() => {
          const r = recipes.find(r => r.id === detailId);
          if (!r) return null;
          return <DesktopDetail r={r} produtos={produtos} negocioNome={negocioNome} onEdit={() => openEditR(r.id)} onCopy={() => copiarReceita(r.id)} onDelete={() => pedirExcR(r.id)} />;
        })()}
        {view === "rForm" && (
          <div style={{maxWidth:720,margin:"0 auto",width:"100%",padding:"20px 28px 80px"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
              <button style={{...s.bback,background:RC,color:V}} onClick={() => setView(editId?"detail":"list")}>← Cancelar</button>
              <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:20,color:V}}>{editId?"Editar Receita":"Nova Receita"}</h2>
            </div>
            <ReceitaForm />
          </div>
        )}
        {view === "pForm" && (
          <div style={{maxWidth:600,margin:"0 auto",width:"100%",padding:"20px 28px 80px"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
              <button style={{...s.bback,background:RC,color:V}} onClick={() => setView("list")}>← Cancelar</button>
              <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:20,color:V}}>{editPId?"Editar Produto":"Novo Produto"}</h2>
            </div>
            <ProdutoForm />
          </div>
        )}
  ...
  return (
    <>
      ...
      <QuickProdModal quickP={quickP} setQuickP={setQuickP} produtos={produtos} saveP={saveP} toast_={toast_}/>
      <RelModal />
