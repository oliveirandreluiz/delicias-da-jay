...
// ─── ESTILOS (objeto estático — não recria a cada render) ─────────────────
const s = {
...
// ─── CSS GLOBAL (injetado uma vez via useEffect no App, não inline) ───────
const GLOBAL_CSS = `
...
// ═══════════════════════════════════════════════════════════════
// COMPONENTES DESACOPLADOS (estado local isolado — não re-renderiza App)
// ═══════════════════════════════════════════════════════════════
...
function ProdutoForm({ initialData, editId, recipes, saving, onSaved, onCancel, onDelete, onCopy, toast_ }) {
  const [f, setF] = useState({ nome:"", preco_ultimo:"", embalagem_qtd:"", unidade:"g", categoria:"Secos", tipo:"ambos" });
  useEffect(() => { if (initialData) setF({ ...initialData, preco_ultimo: String(initialData.preco_ultimo ?? ""), embalagem_qtd: String(initialData.embalagem_qtd ?? "") }); }, [initialData?.id]); // eslint-disable-line
  ...
  return (
    <div style={s.fb}>
      <div style={s.sec}>
        ...
        <select style={s.inp} value={f.categoria} onChange={e => handleChange("categoria", e.target.value)}>
          {CAT_P.map(c => <option key={c}>{c}</option>)}
        </select>
        <div style={{display:"flex",gap:10}}>
          <div style={{flex:1}}>
            <label style={s.lbl}>Preço pago (R$) *</label>
            <input style={s.inp} type="number" step="0.01" placeholder="6.39" value={f.preco_ultimo} onChange={e => handleChange("preco_ultimo", e.target.value)}/>
          </div>
          <div style={{flex:1}}>
            <label style={s.lbl}>Unidade</label>
            <select style={s.inp} value={f.unidade} onChange={e => handleChange("unidade", e.target.value)}>
...
function ReceitaForm({ initialData, editId, produtos, saving, onSaved, toast_ }) {
  ...
  useEffect(() => { if (initialData) setF(JSON.parse(JSON.stringify(initialData))); }, [initialData?.id]); // eslint-disable-line
  ...
  const handleSave = () => {
    if (!f.nome.trim()) { toast_("⚠️ Informe o nome"); return; }
    onSaved({ ...f, ingredientes: f.ingredientes.filter(i => i.produtoId) });
  };
  ...
  return (
    <div style={s.fb}>
      <div style={s.sec}>
        ...
        <label style={s.lbl}>Nome *</label>
        <input style={s.inp} placeholder="Ex: Brownie Ninho" value={f.nome} onChange={e => handleChange("nome",e.target.value)}/>
        <label style={s.lbl}>Categoria</label>
        <select style={s.inp} value={f.categoria} onChange={e => handleChange("categoria",e.target.value)}>
        ...
        <input style={s.inp} type="number" placeholder="10" value={f.rendimento} onChange={e => handleChange("rendimento",parseFloat(e.target.value)||0)}/>
        <label style={s.lbl}>Observações (opcional)</label>
        <textarea style={{...s.inp,height:65,resize:"none"}} value={f.obs||""} onChange={e => handleChange("obs",e.target.value)}/>
      </div>
      <div style={s.sec}>
        ...
        {f.ingredientes.map((ing, idx) => {
          ...
          const preco = p ? (p.preco_ultimo ?? p.preco ?? 0) : 0;
          const embQtd = p ? (p.embalagem_qtd ?? p.embalagemQtd ?? 1) : 1;
          ...
          return (
            <div key={idx} style={s.ic}>
              <div style={s.ich}>
                ...
                <button style={s.brm} onClick={() => handleRemoveIng(idx)}>✕ remover</button>
              </div>
              ...
              <div style={{position:"relative",marginBottom:p?0:10}}>
                {p ? (
                  <div style={{display:"flex",alignItems:"center",gap:7,background:"#F0FFF4",border:"2px solid #86EFAC",borderRadius:9,padding:"9px 12px"}}>
                    ...
                    <button style={{background:"none",border:"none",color:R,fontSize:15,cursor:"pointer",padding:0,flexShrink:0}} onClick={() => handleIngChange(idx,"produtoId","")}>✕</button>
                  </div>
                ) : (
                  <>
                    <input style={{...s.inp,marginBottom:0,paddingLeft:34,background:CR,borderColor:RC}} placeholder="Digite para buscar..." value={bsc} onChange={e => handleIngChange(idx,"_busca",e.target.value)}/>
                    ...
                    {res.length > 0 && (
                      <div style={{position:"absolute",top:"100%",left:0,right:0,background:W,border:`2px solid ${RC}`,borderRadius:10,zIndex:50,boxShadow:"0 8px 20px rgba(74,26,44,.15)",maxHeight:190,overflowY:"auto"}}>
                        {res.map(pr => (
                          <div key={pr.id} className="po" style={{padding:"9px 12px",cursor:"pointer",borderBottom:`1px solid #FDE8ED`,fontSize:12}} onClick={() => handleIngChange(idx,"produtoId",pr.id)}>
                            <div style={{fontWeight:600,color:V}}>{pr.nome}</div>
                            <div style={{fontSize:10,color:G,marginTop:1}}>{pr.categoria} · R$ {fmtN(pr.preco_ultimo ?? pr.preco ?? 0)}/{pr.embalagem_qtd ?? pr.embalagemQtd ?? 1}{pr.unidade}</div>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {p && <>
                ...
                <input style={s.inp} type="number" step="any" placeholder="300" value={ing.usadoQtd||""} onChange={e => handleIngChange(idx,"usadoQtd",parseFloat(e.target.value)||0)}/>
      ...
      <div style={s.sec}>
        ...
        <div style={{display:"flex",gap:10}}>
          <div style={{flex:1}}>
            ...
            <input style={{...s.inp,fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:R,textAlign:"center"}} type="number" min={0} step={1} placeholder="100" value={f.margem||""} onChange={e => handleChange("margem",parseFloat(e.target.value)||0)}/>
          </div>
          <div style={{flex:1}}>
            <label style={s.lbl}>Taxa delivery (%)</label>
            <input style={{...s.inp,fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:700,color:R,textAlign:"center"}} type="number" min={0} step={1} placeholder="30" value={f.taxaDelivery||""} onChange={e => handleChange("taxaDelivery",parseFloat(e.target.value)||0)}/>
          </div>
        </div>
        <label style={s.lbl}>Outros custos — gás, energia, etc. (%)</label>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
          <input style={{...s.inp,marginBottom:0,width:90,fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700,color:R,textAlign:"center"}} type="number" min={0} step={1} placeholder="30" value={f.outrosCustos!==undefined?f.outrosCustos:30} onChange={e => handleChange("outrosCustos",parseFloat(e.target.value)||0)}/>
        ...
        <input style={s.inp} type="number" step="0.01" placeholder="0,00" value={f.despesas||""} onChange={e => handleChange("despesas",parseFloat(e.target.value)||0)}/>
        <div style={s.ph}>
        ...
        <input style={s.inp} type="number" step="0.01" placeholder="0,00" value={f.precoApp||""} onChange={e => handleChange("precoApp",parseFloat(e.target.value)||0)}/>
      </div>
...
function CompraForm({ produtos, saving, onSaved, toast_ }) {
  const [header, setHeader] = useState({ data_compra: new Date().toISOString().split("T")[0], fornecedor:"", forma_pagamento:"Pix", obs:"" });
  ...
  const handleSelectProduto = (idx, prod) => handleItemChange(idx, "produto_id", prod.id);
  ...
  return (
    <div style={s.fb}>
      <div style={s.sec}>
        ...
        <div style={{display:"flex",gap:10}}>
          <div style={{flex:1}}>
            ...
            <input style={s.inp} type="date" value={header.data_compra} onChange={e => handleHeaderChange("data_compra", e.target.value)}/>
          </div>
          <div style={{flex:1}}>
            <label style={s.lbl}>Pagamento</label>
            <select style={s.inp} value={header.forma_pagamento} onChange={e => handleHeaderChange("forma_pagamento", e.target.value)}>
              <option value="">—</option>
        ...
        <input style={s.inp} placeholder="Ex: Atacadão" value={header.fornecedor} onChange={e => handleHeaderChange("fornecedor", e.target.value)}/>
      </div>
      <div style={s.sec}>
        <div style={s.st}>📦 Itens da Compra</div>
        <div style={s.help}>Busque o produto cadastrado. Se não encontrar, cadastre na aba Produtos.</div>
        {itens.map((item, idx) => {
          ...
          return (
            <div key={idx} style={s.ic}>
              <div style={s.ich}>
                <span style={{fontSize:12,fontWeight:600,color:G}}>Item {idx+1}</span>
                <button style={s.brm} onClick={() => handleRemoveItem(idx)}>✕ remover</button>
              </div>
              ...
              <div style={{position:"relative",marginBottom:prod?0:10}}>
                {prod ? (
                  <div style={{display:"flex",alignItems:"center",gap:7,background:"#F0FFF4",border:"2px solid #86EFAC",borderRadius:9,padding:"9px 12px"}}>
                    ...
                    <button style={{background:"none",border:"none",color:R,fontSize:15,cursor:"pointer",padding:0,flexShrink:0}} onClick={() => handleItemChange(idx,"produto_id","")}>✕</button>
                  </div>
                ) : (
                  <>
                    <input style={{...s.inp,marginBottom:0,paddingLeft:34,background:CR,borderColor:RC}} placeholder="Buscar produto..." value={bsc} onChange={e => handleItemChange(idx,"_busca",e.target.value)}/>
                    <span style={{position:"absolute",left:11,top:"50%",transform:"translateY(-50%)",fontSize:13,pointerEvents:"none"}}>🔍</span>
                    {res.length > 0 && (
                      <div style={{position:"absolute",top:"100%",left:0,right:0,background:W,border:`2px solid ${RC}`,borderRadius:10,zIndex:50,boxShadow:"0 8px 20px rgba(74,26,44,.15)",maxHeight:190,overflowY:"auto"}}>
                        {res.map(pr => (
                          <div key={pr.id} className="po" style={{padding:"9px 12px",cursor:"pointer",borderBottom:`1px solid #FDE8ED`,fontSize:12}} onClick={() => { handleSelectProduto(idx, pr); handleItemChange(idx,"unidade",pr.unidade); }}>
                            <div style={{fontWeight:600,color:V}}>{pr.nome} <span style={{fontWeight:400,color:G}}>({pr.codigo})</span></div>
                            <div style={{fontSize:10,color:G,marginTop:1}}>{pr.categoria} · último: {fmt(pr.preco_ultimo)}</div>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {prod && <>
                ...
                <div style={{display:"flex",gap:10}}>
                  <div style={{flex:1}}>
                    ...
                    <input style={s.inp} type="number" step="any" placeholder="2" value={item.qtd} onChange={e => handleItemChange(idx,"qtd",e.target.value)}/>
                  </div>
                  <div style={{flex:1}}>
                    <label style={s.lbl}>Unidade</label>
                    <select style={s.inp} value={item.unidade} onChange={e => handleItemChange(idx,"unidade",e.target.value)}>
                      {UNIDS.map(u => <option key={u}>{u}</option>)}
                    </select>
                  </div>
                </div>
                <label style={s.lbl}>Valor unitário (R$) *</label>
                <input style={s.inp} type="number" step="0.01" placeholder="42.00" value={item.valor_unitario} onChange={e => handleItemChange(idx,"valor_unitario",e.target.value)}/>
      ...
      <div style={s.ph}>
        <div style={s.phl}>🧾 Total da Compra</div>
        <div style={s.phv}>{fmt(totalItens)}</div>
        <div style={{fontSize:11,opacity:.75,marginTop:2}}>{itens.filter(i=>i.produto_id).length} itens</div>
      </div>
      <label style={s.lbl}>Observações (opcional)</label>
      <textarea style={{...s.inp,height:65,resize:"none"}} value={header.obs||""} onChange={e => handleHeaderChange("obs",e.target.value)}/>
...
// ═══════════════════════════════════════════════════════════════
// APP PRINCIPAL (orquestração — sem estado de formulário)
// ═══════════════════════════════════════════════════════════════
export default function App() {
  // ── CSS injection (uma vez) ──
  useEffect(() => {
    const id = "delicias-jay-css";
  ...
  const [pFormInit, setPFormInit] = useState(null);
  const [rFormInit, setRFormInit] = useState(null);
  ...
  const [mesFiltroC, setMesFiltroC] = useState("Todos");
  ...
  // ── MEMOIZED CALCS (evita recalcular em cada render) ──
  const recipesCalc = useMemo(() =>
    recipes.map(r => ({ ...r, _calc: calc(r, produtos) })),
    [recipes, produtos]
  );
  ...
  async function loadUserData(sessionUser) {
    ...
    try {
      ...
      if (mRes.data) {
        const map = {};
        mRes.data.forEach(m => { map[m.user_id] = m.user_id === sessionUser.id ? (sessionUser.email?.split("@")[0] || "Eu") : "Membro"; });
        setMapaUsuarios(map);
      }
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "SIGNED_OUT") { setUser(null); setNegocioId(null); setNegocioNome(""); setRecipes([]); setProdutos([]); setCompras([]); setReceitasRowId(null); setView("list"); setTab("receitas"); setAppReady(true); return; }
      if ((event === "SIGNED_IN" || event === "INITIAL_SESSION") && session?.user) { setUser(session.user); await loadUserData(session.user); return; }
  ...
  const reloadProdutos = useCallback(async () => {
    const { data } = await supabase.from("produtos_v2").select("*").eq("negocio_id", negocioId).eq("ativo", true).order("nome");
    if (data) setProdutos(data.map(p => ({ ...p, preco: p.preco_ultimo, embalagemQtd: p.embalagem_qtd })));
  }, [negocioId]);

  const reloadCompras = useCallback(async () => {
    const { data } = await supabase.from("vw_compras_listagem").select("*").eq("negocio_id", negocioId).order("data_compra", { ascending: false }).limit(100);
  ...
  const fazerCadastro = async () => {
    ...
    if (authForm.senha.length < 6) { setAuthError("Senha: mín. 6 caracteres ⚠️"); return; }
    setAuthLoading(true); setAuthError("");
  ...
  const handleSaveProd = async (data) => {
    setSaving(true);
    const payload = { nome: data.nome.trim(), categoria: data.categoria, unidade: data.unidade, tipo: data.tipo||"ambos", preco_ultimo: data.preco_ultimo, embalagem_qtd: data.embalagem_qtd };
    try {
      if (editPId) await supabase.from("produtos_v2").update(payload).eq("id", editPId);
      else { payload.negocio_id = negocioId; await supabase.from("produtos_v2").insert(payload); }
  ...
  const pedirExcP = id => {
    ...
    setConfirmDel({ tipo:"produto", id, nome: produtos.find(x=>x.id===id)?.nome||"" });
  };
  const copiarProduto = id => {
    const p = produtos.find(x => x.id === id);
    setPFormInit({ ...p, nome: `Cópia de ${p.nome}`, id: genId() }); setEditPId(null); setView("pForm"); toast_("📋 Revise e salve!");
  };
  ...
  const copiarReceita = id => {
    ...
    setRFormInit({ ...JSON.parse(JSON.stringify(r)), id: genId(), nome: `Cópia de ${r.nome}` }); setEditId(null); setView("rForm"); toast_("📋 Revise e salve!");
  };

  // ── COMPRA CRUD ──
  const openNewC = () => setView("cForm");
  const openDetC = async (id) => {
    const { data: itens } = await supabase.from("compra_itens").select("*").eq("compra_id", id).order("created_at");
    setCompraItens(prev => ({ ...prev, [id]: itens || [] }));
    setCompraDetalhe(id); setView("cDetail");
  };
  const handleSaveCompra = async (header, validItens) => {
    setSaving(true);
    try {
      const { data: newC, error } = await supabase.from("compras").insert({
        negocio_id: negocioId, data_compra: header.data_compra,
        fornecedor: header.fornecedor || null, forma_pagamento: header.forma_pagamento || null,
        obs: header.obs || null, created_by: user.id,
      }).select("id").single();
      if (error) throw error;
      const payload = validItens.map(i => ({ compra_id: newC.id, produto_id: i.produto_id, qtd: parseFloat(i.qtd)||0, unidade: i.unidade||"un", valor_unitario: parseFloat(i.valor_unitario)||0 }));
      await supabase.from("compra_itens").insert(payload);
  ...
  const confirmarExc = async () => {
    ...
    if (confirmDel.tipo === "receita") {
      ...
      await supabase.from("produtos_v2").update({ ativo: false }).eq("id", confirmDel.id);
      await reloadProdutos(); toast_("🗑 Produto desativado");
    } else if (confirmDel.tipo === "compra") {
      await supabase.from("compras").delete().eq("id", confirmDel.id);
      await reloadCompras(); toast_("🗑 Compra excluída"); setView("list"); setTab("compras");
    }
  ...
  const exportarCSV = (tipo) => {
    let csv = "", nome = "";
    if (tipo === "receitas") {
      nome = "receitas.csv"; csv = "Nome;Categoria;Rendimento;Margem%;Taxa%;Custo Total;Custo/Un;Preço Sugerido;Preço App;Lucro Lote\n";
      ...
      nome = "produtos.csv"; csv = "Código;Nome;Categoria;Unidade;Preço;Embalagem;Custo/Un\n";
  ...
  if (!user && authView === "emailConfirm") return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:V}}>
      <div style={{textAlign:"center",padding:"0 32px",width:"100%",maxWidth:380}}>
        ...
        <div style={{color:`${RL}bb`,fontSize:14,lineHeight:1.7,marginBottom:28}}>Enviamos um link para<br/><strong style={{color:RL}}>{authForm.email}</strong><br/><br/>Confirme e volte para entrar.</div>
  ...
  if (!user && authView === "cadastro") return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:V}}>
      <div style={{textAlign:"center",padding:"0 32px",width:"100%",maxWidth:380}}>
        ...
        <input style={{width:"100%",padding:"13px 18px",borderRadius:50,border:`2px solid ${authError?"#ff6b6b":"#C45C7444"}`,background:"rgba(255,255,255,.12)",fontSize:14,color:W,WebkitTextFillColor:W,outline:"none",textAlign:"center",boxSizing:"border-box",marginBottom:10,fontFamily:"'DM Sans',sans-serif"}} type="text" placeholder="Nome do seu negócio 🏪" value={authForm.nomeNegocio} onChange={e => { setAuthForm({...authForm, nomeNegocio: e.target.value}); setAuthError(""); }}/>
        <input style={{width:"100%",padding:"13px 18px",borderRadius:50,border:`2px solid ${authError?"#ff6b6b":"#C45C7444"}`,background:"rgba(255,255,255,.12)",fontSize:14,color:W,WebkitTextFillColor:W,outline:"none",textAlign:"center",boxSizing:"border-box",marginBottom:10,fontFamily:"'DM Sans',sans-serif"}} type="email" placeholder="Seu email" value={authForm.email} onChange={e => { setAuthForm({...authForm, email: e.target.value}); setAuthError(""); }}/>
        <input style={{width:"100%",padding:"13px 18px",borderRadius:50,border:`2px solid ${authError?"#ff6b6b":"#C45C7444"}`,background:"rgba(255,255,255,.12)",fontSize:14,color:W,WebkitTextFillColor:W,outline:"none",textAlign:"center",boxSizing:"border-box",marginBottom:10,fontFamily:"'DM Sans',sans-serif"}} type="password" placeholder="Senha (mín. 6 caracteres)" value={authForm.senha} onChange={e => { setAuthForm({...authForm, senha: e.target.value}); setAuthError(""); }} onKeyDown={e => e.key==="Enter"&&fazerCadastro()}/>
  ...
  if (!user) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:V}}>
      <div style={{textAlign:"center",padding:"0 32px",width:"100%",maxWidth:380}}>
        ...
        <input style={{width:"100%",padding:"13px 18px",borderRadius:50,border:`2px solid ${authError?"#ff6b6b":"#C45C7444"}`,background:"rgba(255,255,255,.15)",fontSize:15,color:W,WebkitTextFillColor:W,outline:"none",textAlign:"center",boxSizing:"border-box",marginBottom:10,fontFamily:"'DM Sans',sans-serif"}} type="email" placeholder="Seu email" value={authForm.email} onChange={e => { setAuthForm({...authForm, email: e.target.value}); setAuthError(""); }}/>
        <input style={{width:"100%",padding:"13px 18px",borderRadius:50,border:`2px solid ${authError?"#ff6b6b":"#C45C7444"}`,background:"rgba(255,255,255,.15)",fontSize:15,color:W,WebkitTextFillColor:W,outline:"none",textAlign:"center",letterSpacing:".15em",boxSizing:"border-box",marginBottom:10,fontFamily:"'DM Sans',sans-serif",animation:authError?"shake .3s":""}} type="password" placeholder="Senha" value={authForm.senha} onChange={e => { setAuthForm({...authForm, senha: e.target.value}); setAuthError(""); }} onKeyDown={e => e.key==="Enter"&&fazerLogin()}/>
        {authError && <div style={{color:"#ff8fa3",fontSize:12,marginBottom:10}}>{authError}</div>}
  ...
  // ══════════════════════════════════════════════════════════════
  // FORM VIEWS (delegam pro componente isolado)
  // ══════════════════════════════════════════════════════════════
  if (view === "pForm") return (
    <div style={s.app}>
      <header style={s.hdr}><button style={s.bback} onClick={() => setView("list")}>← Voltar</button><span style={s.logo}>{editPId?"Editar Produto":"Novo Produto"}</span><div style={{width:70}}/></header>
      <ProdutoForm initialData={pFormInit} editId={editPId} recipes={recipes} saving={saving} onSaved={handleSaveProd} onCancel={() => setView("list")} onDelete={pedirExcP} onCopy={copiarProduto} toast_={toast_}/>
      <ModalConfirm item={confirmDel} onConfirm={confirmarExc} onCancel={() => setConfirmDel(null)}/>
      {toast && <div style={s.tst}>{toast}</div>}
    </div>
  );

  if (view === "cForm") return (
    <div style={s.app}>
      <header style={s.hdr}><button style={s.bback} onClick={() => { setView("list"); setTab("compras"); }}>← Cancelar</button><span style={s.logo}>Nova Compra</span><div style={{width:70}}/></header>
      <CompraForm produtos={produtos} saving={saving} onSaved={handleSaveCompra} toast_={toast_}/>
      {toast && <div style={s.tst}>{toast}</div>}
    </div>
  );

  if (view === "rForm") return (
    <div style={s.app}>
      <header style={s.hdr}><button style={s.bback} onClick={() => setView(editId?"detail":"list")}>← Cancelar</button><span style={s.logo}>{editId?"Editar Receita":"Nova Receita"}</span><div style={{width:70}}/></header>
      <ReceitaForm initialData={rFormInit} editId={editId} produtos={produtos} saving={saving} onSaved={handleSaveRecipe} toast_={toast_}/>
      <ModalConfirm item={confirmDel} onConfirm={confirmarExc} onCancel={() => setConfirmDel(null)}/>
      {toast && <div style={s.tst}>{toast}</div>}
    </div>
  );
  ...
  if (view === "cDetail" && compraDetalhe) {
    const c = compras.find(x => x.id === compraDetalhe);
    if (!c) { setView("list"); return null; }
    const itens = compraItens[compraDetalhe] || [];
    const total = itens.reduce((sum, i) => sum + parseFloat(i.valor_subtotal||0), 0);
    return (
      <div style={s.app}>
        <div style={s.dh}>
          <button style={s.bback} onClick={() => { setView("list"); setTab("compras"); }}>← Voltar</button>
          <div style={{fontSize:44,marginBottom:7,marginTop:10}}>🛒</div>
          <div style={s.dn}>{c.fornecedor || "Compra"}</div>
          <div style={s.dc}>{new Date(c.data_compra+"T12:00:00").toLocaleDateString("pt-BR")} · {c.forma_pagamento||""} · {mapaUsuarios[c.created_by]||""}</div>
        </div>
        <div style={s.db}>
          <div style={s.sec}>
            <div style={s.st}>📦 Itens ({itens.length})</div>
            {itens.map((i, idx) => <div key={idx} style={s.ir}><div><div style={{fontWeight:500,fontSize:13}}>{i.produto_nome_snapshot}</div><div style={{fontSize:11,color:G,marginTop:1}}>{i.qtd} {i.unidade} × {fmt(i.valor_unitario)}</div></div><div style={s.icost}>{fmt(i.valor_subtotal)}</div></div>)}
            <div style={{...s.ir,borderTop:`2px solid ${RC}`,marginTop:5,paddingTop:7,fontWeight:700}}><div>Total</div><div style={s.icost}>{fmt(total)}</div></div>
          </div>
          {c.obs && <div style={s.sec}><div style={s.st}>📝 Obs</div><div style={{fontSize:13,color:G,lineHeight:1.6}}>{c.obs}</div></div>}
          <button style={s.bd} onClick={() => pedirExcC(c.id)}>🗑 Excluir compra</button>
        </div>
        <ModalConfirm item={confirmDel} onConfirm={confirmarExc} onCancel={() => setConfirmDel(null)}/>
        {toast && <div style={s.tst}>{toast}</div>}
      </div>
  ...
  // ── DETALHE RECEITA ──
  if (view === "detail") {
    const rc = recipesCalc.find(x => x.id === detailId);
    if (!rc) { setView("list"); return null; }
    ...
    return (
      <div style={s.app}>
        <div style={s.dh}>
          ...
          <div style={{fontSize:44,marginBottom:7,marginTop:10}}>{rc.emoji}</div>
          <div style={s.dn}>{rc.nome}</div>
          <div style={s.dc}>{rc.categoria||"Sem categoria"} · {rc.rendimento} un.</div>
        </div>
        <div style={s.db}>
          <div style={s.sec}>
            <div style={s.st}>🧂 Ingredientes</div>
            {rc.ingredientes.map((ing,i) => { const p = produtos.find(x=>x.id===ing.produtoId); if (!p) return null; const pr=p.preco_ultimo??p.preco??0; const eq=p.embalagem_qtd??p.embalagemQtd??1; return (
              <div key={i} style={s.ir}><div><div style={{fontWeight:500,fontSize:13}}>{p.nome}</div><div style={{fontSize:11,color:G,marginTop:1}}>{ing.usadoQtd}{p.unidade} · {fmt(pr)}/{eq}{p.unidade}</div></div><div style={s.icost}>{fmt(eq>0?(pr/eq)*ing.usadoQtd:0)}</div></div>
            );})}
            ...
            <div style={s.ir}><div>Outros custos ({rc.outrosCustos||30}%)</div><div style={s.icost}>{fmt(outros)}</div></div>
            {rc.despesas > 0 && <div style={s.ir}><div>Despesas extras</div><div style={s.icost}>{fmt(rc.despesas)}</div></div>}
            <div style={{...s.ir,fontWeight:700}}><div>Custo total</div><div style={s.icost}>{fmt(total)}</div></div>
          </div>
          <div style={s.sec}>
            <div style={s.st}>💰 Precificação</div>
            <div style={s.pg}>
              <div style={s.pb}><div style={s.pl}>Custo/un</div><div style={s.pv}>{fmt(porUn)}</div></div>
              <div style={s.pb}><div style={s.pl}>Margem</div><div style={s.pv}>{rc.margem}%</div></div>
              <div style={s.pb}><div style={s.pl}>Preço s/ taxa</div><div style={s.pv}>{fmt(semT)}</div></div>
              <div style={s.pb}><div style={s.pl}>Taxa ({rc.taxaDelivery}%)</div><div style={s.pv}>{fmt(taxa)}</div></div>
            </div>
            <div style={s.ph}><div style={s.phl}>🏷 Preço de venda/un</div><div style={s.phv}>{fmt(f_)}</div></div>
            {rc.precoApp > 0 && <div style={{...s.pb,background:"#F0FFF4",border:"1.5px solid #86EFAC",marginBottom:7}}><div style={{...s.pl,color:"#166534"}}>✅ Preço praticado</div><div style={{...s.pv,color:"#166534"}}>{fmt(rc.precoApp)}</div></div>}
          ...
          <div style={{display:"flex",gap:8,marginTop:4,flexWrap:"wrap"}}>
            <button style={s.be} onClick={() => openEditR(rc.id)}>✏️ Editar</button>
            <button style={{...s.bpri,padding:"13px 14px"}} onClick={() => copiarReceita(rc.id)}>📋 Copiar</button>
            <button style={s.bd} onClick={() => pedirExcR(rc.id)}>🗑 Excluir</button>
          </div>
        </div>
        <ModalConfirm item={confirmDel} onConfirm={confirmarExc} onCancel={() => setConfirmDel(null)}/>
        {toast && <div style={s.tst}>{toast}</div>}
  ...
  return (
    <div style={s.app}>
      ...
      {tab === "receitas" && <>
        ...
        {filtR.length===0
          ? <div style={s.emp}><div style={{fontSize:46,marginBottom:10}}>🍰</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:17,marginBottom:5}}>{recipes.length===0?"Nenhuma receita":"Nada encontrado"}</div></div>
          : <div style={s.list}>{filtR.map(r => <div key={r.id} style={s.card} className="ch" onClick={() => openDet(r.id)}>
              ...
              <div style={s.cpr}><div style={s.cpv}>{fmt(r._calc.final)}</div><div style={s.cpl}>p/ unidade</div></div>
            </div>)}</div>
        }
      </>}
      ...
      {tab === "produtos" && <>
        ...
        <div style={s.cnt}>{filtP.length} produto{filtP.length!==1?"s":""}</div>
        <div style={s.list}>{filtP.map(p => {
          const used = recipes.filter(r => r.ingredientes.some(i => i.produtoId === p.id)).length;
          const pu = p.embalagem_qtd ? p.preco_ultimo / p.embalagem_qtd : 0;
          return <div key={p.id} style={s.card} className="ch" onClick={() => openEditP(p.id)}>
            <div style={{...s.cem,fontSize:20}}>{CATEMOJI[p.categoria]||"📦"}</div>
            <div style={s.cbd}><div style={s.cnm}>{p.nome}</div><div style={s.cmt}>{p.codigo} · {p.categoria} · R$ {fmtN(pu)}/{p.unidade}</div>{used>0 && <div style={{fontSize:10,color:R,fontWeight:600,marginTop:2}}>usado em {used} receita{used>1?"s":""}</div>}</div>
            <div style={s.cpr}><div style={s.cpv}>{fmt(p.preco_ultimo)}</div><div style={s.cpl}>{p.embalagem_qtd}{p.unidade}</div></div>
          </div>;
        })}</div>
      </>}

      {tab === "compras" && <>
        <div style={s.srchW}><span style={{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",fontSize:13}}>🔍</span><input style={s.srch} placeholder="Buscar por fornecedor..." value={searchC} onChange={e => setSearchC(e.target.value)}/></div>
        <div style={s.chips}>
          <button style={{...s.chip,...(mesFiltroC==="Todos"?s.chipa:{})}} onClick={() => setMesFiltroC("Todos")}>Todos</button>
          {mesesCompras.map(m => { const [a,me]=m.split("-"); return <button key={m} style={{...s.chip,...(mesFiltroC===m?s.chipa:{})}} onClick={() => setMesFiltroC(m)}>{["","Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"][parseInt(me)]}/{a.slice(2)}</button>; })}
        </div>
        <div style={{padding:"4px 18px 8px",display:"flex",justifyContent:"space-between"}}>
          <span style={{fontSize:11,color:`${G}88`}}>{filtC.length} compra{filtC.length!==1?"s":""}</span>
          <span style={{fontSize:13,fontWeight:700,color:R}}>Total: {fmt(filtC.reduce((sum,c) => sum + parseFloat(c.valor_total||0), 0))}</span>
        </div>
        {filtC.length===0
          ? <div style={s.emp}><div style={{fontSize:46,marginBottom:10}}>🛒</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:17,marginBottom:5}}>{compras.length===0?"Nenhuma compra":"Nada encontrado"}</div></div>
          : <div style={s.list}>{filtC.map(c => <div key={c.id} style={s.card} className="ch" onClick={() => openDetC(c.id)}>
              <div style={{...s.cem,fontSize:20}}>🛒</div>
              <div style={s.cbd}><div style={s.cnm}>{c.fornecedor||"Sem fornecedor"}</div><div style={s.cmt}>{new Date(c.data_compra+"T12:00:00").toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})} · {c.forma_pagamento||""}{mapaUsuarios[c.created_by]?` · ${mapaUsuarios[c.created_by]}`:""}</div></div>
              <div style={s.cpr}><div style={s.cpv}>{fmt(c.valor_total)}</div><div style={s.cpl}>total</div></div>
            </div>)}</div>
        }
      </>}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:400,display:"flex",alignItems:"flex-end"}}>
          <div style={{background:W,borderRadius:"18px 18px 0 0",padding:20,width:"100%",maxWidth:480,margin:"0 auto",maxHeight:"85vh",overflowY:"auto"}}>
            ...
            <div style={{...s.sec,marginBottom:10}}>
              <div style={s.st}>🍰 Receitas ({recipesCalc.length})</div>
              {recipesCalc.map(r => <div key={r.id} style={{borderBottom:`1px solid #FDE8ED`,padding:"7px 0",fontSize:12}}>
                <div style={{display:"flex",justifyContent:"space-between",fontWeight:600}}><span>{r.emoji} {r.nome}</span><span style={{color:R}}>{fmt(r._calc.final)}/un</span></div>
                <div style={{display:"flex",justifyContent:"space-between",color:G,marginTop:2}}><span>Custo {fmt(r._calc.porUn)}/un · Margem {r.margem}%</span><span style={{color:"#92400E"}}>Lucro {fmt(r._calc.lucro)}</span></div>
              </div>)}
              <button style={{...s.bsave,marginTop:12,padding:12,fontSize:13}} onClick={() => exportarCSV("receitas")}>⬇️ Exportar Receitas CSV</button>
            </div>
            <div style={s.sec}>
              <div style={s.st}>📦 Produtos ({filtP.length})</div>
              {filtP.slice(0,20).map(p => { const pu = p.embalagem_qtd ? p.preco_ultimo/p.embalagem_qtd : 0; return (
                <div key={p.id} style={{borderBottom:`1px solid #FDE8ED`,padding:"6px 0",fontSize:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontWeight:600}}><span>{p.nome}</span><span style={{color:R}}>{fmt(p.preco_ultimo)}</span></div>
                  <div style={{color:G,marginTop:1}}>R$ {fmtN(pu)}/{p.unidade}</div>
                </div>
              );})}
